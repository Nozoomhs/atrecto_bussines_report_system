...
RiskAgent
---------
Consumes a prompt produced by PromptParser for the "risks_blockers" task and
returns a strict JSON dict that matches the schema expected by your pipeline.

Design mirrors unresolved_agent.py:
- Backend-agnostic (LLMBackend.complete(prompt) -> str)
...
ALLOWED_LABELS = {"risk_present", "blocker_present",  "none"}
...
class RiskOutput(BaseModel):
    ...
    def reason_len(cls, v: str) -> str:
        ...
        if len(v) > 100:  # keep consistent with unresolved_agent.py
    ...
    def run(self, prompt: str) -> Dict[str, Any]:
        ...
        for _ in range(self.max_retries + 1):
            try:
                ...
                obj = RiskOutput.model_validate(payload)
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="Run RiskAgent on a prebuilt prompt JSON")
    ap.add_argument("--from_file", type=str, required=True)
    ...
    agent = RiskAgent(backend=backend, max_retries=args.retries)
