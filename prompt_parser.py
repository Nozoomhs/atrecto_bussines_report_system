...
This is the POC skeleton focused on I/O contracts and formatting only.
You can plug these prompts directly into your agent runners. Later you can
swap the agent templates with your final role/instructions without touching
the parsing logic.
...
- Times are rendered in a chosen timezone (default 'Europe/Budapest') as ISO-8601.
...
Note: This module deliberately avoids heavy deps (RAG / embeddings). It's just parsing & formatting.
"""
...
class PromptParser:
    ...
    def build_all_prompts(self) -> Dict[str, Dict[str, str]]:
        ...
        agent_key in {"cost_considerations", "unresolved_items", "risks_blockers", "gross_negligence"}
        """
        self._lazy_load()
    ...
    def build_prompts_for_thread(self, thread_id: str | int) -> Dict[str, str]:
        ...
        return {
            "cost_considerations": self._render_cost_prompt(participants_block, messages_block, thread_id),
            "unresolved_items": self._render_unresolved_prompt(participants_block, messages_block, thread_id),
            "risks_blockers": self._render_risks_prompt(participants_block, messages_block, thread_id),
            "gross_negligence": self._render_negligence_prompt(participants_block, messages_block, thread_id),
        }
    ...
    def _lazy_load(self) -> None:
        ...
        # # Standardize utc as pandas Timestamp
        # if "utc" in self._messages_df.columns:
        #     self._messages_df["date_utc"] = pd.to_datetime(self._messages_df["date_utc"], errors="coerce", utc=True)
    ...
    def _iter_threads(self) -> Iterable[Tuple[str, pd.DataFrame]]:
